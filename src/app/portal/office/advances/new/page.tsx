...
export default function NewAdvance() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    router.push("/portal/office/advances");
  };

  return (
    <div className="min-h-screen bg-muted/20 pb-8">
      <header className="sticky top-0 z-10 border-b bg-background/95 backdrop-blur">
        <div className="flex items-center gap-2 px-4 py-3"><Button asChild size="icon" variant="ghost"><Link href="/portal/office/advances"><ArrowLeft className="h-5 w-5" /></Link></Button><h1 className="text-base font-bold">{t("newAdvance", lang)}</h1></div>
      </header>
