...
export default function OfficeDocs() {
  ...
  const upload = async (e: React.FormEvent) => {
    e.preventDefault(); if (!profile || !file) return; setBusy(true);
    try {
      ...
      toast.success(t("uploaded", lang)); setFile(null); setDocNumber(""); setExpiresOn(""); await load();
  ...
  return (
    <div className="min-h-screen bg-muted/20 pb-8">
      <header className="sticky top-0 z-10 border-b bg-background/95 backdrop-blur">
        <div className="flex items-center gap-2 px-4 py-3"><Button asChild size="icon" variant="ghost"><Link href="/portal/office"><ArrowLeft className="h-5 w-5" /></Link></Button><h1 className="text-base font-bold">{t("documents", lang)}</h1></div>
      </header>
      <main className="mx-auto max-w-md space-y-3 px-4 pt-4">
        ...
        <div className="space-y-2">{docs.map(d => (<Card key={d.id} className="p-3"><div className="flex items-center justify-between gap-2"><p className="text-sm font-semibold truncate">{t(("doc_" + d.doc_type) as any, lang)}</p><span className="rounded bg-muted px-2 py-1 text-xs whitespace-nowrap">{t((d.status === "verified" ? "verified" : d.status === "rejected" ? "rejected" : "pending") as any, lang)}</span></div>{d.doc_number && <p className="text-xs text-muted-foreground">{d.doc_number}</p>}{d.expires_on && <p className="text-xs text-muted-foreground">{t("expiresOn", lang)}: {d.expires_on}</p>}</Card>))}</div>
      </main>
    </div>
  );
}
