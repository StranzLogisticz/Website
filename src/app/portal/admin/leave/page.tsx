...
export default function AccountsLeaveList() {
  ...
  const load = async () => {
    ...
    if (data) setRequests(data as LeaveRow[]);
    setLoading(false);
  };
  ...
  const cancel = async (id: string) => {
    ...
    toast.success(t("cancelled", lang));
    load();
  };
      ...
      <header className="sticky top-0 z-10 border-b bg-background/95 backdrop-blur">
        <div className="flex items-center justify-between gap-2 px-4 py-3">
          <div className="flex items-center gap-2">
            <Button asChild size="icon" variant="ghost"><Link href="/portal/admin" aria-label={t("back", lang)}><ArrowLeft className="h-5 w-5" /></Link></Button>
            <h1 className="text-base font-bold">{t("myLeaves", lang)}</h1>
          </div>
          <Button asChild size="sm"><Link href="/portal/admin/leave/new"><CalendarPlus className="mr-1 h-4 w-4" />{t("applyLeave", lang)}</Link></Button>
        </div>
      </header>
